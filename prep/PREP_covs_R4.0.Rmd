...
#extract bmi and race, household income from covs df
covs <- covs %>% 
  select(src_subject_id, eventname, sex, bmi, household_income, race.6level)
...
Double check household income variable characteristics using tabulated data from release 4.0

Income data is only available at baseline 

```{r, family income info}

demo <- parent_mh <- rio::import("/Volumes/GenScotDepression/data/abcd/release4.0/iii.data/Mental_Health/pdem02.rds")

demo <- demo %>% 
  select(src_subject_id, eventname, demo_comb_income_v2)

#convert to wide to check eventname var and availability of data across timepoints 
...
covs_wide <- covs_wide %>% 
  rename( bmi_y0 = bmi.baseline_year_1_arm_1, 
          ...
          household_income_y0 = household_income.baseline_year_1_arm_1,
...
covs_wide <- covs_wide %>% 
  select(src_subject_id, bmi_y0, bmi_y1, bmi_y2, household_income_y0, sex, race_y0, race_6m, race_y1, race_18m, race_y2)
