...
#import t1 to get ICV (Intracranial volume) measure 
#Note:
  # we have extracted this in the imaging qc but we will pull out again here
  #for our main analysis we will need Year 2 ICV measure (we won't load data this at present)
...
#extract ICV from t1 dataframe
ICV <-  t1  %>% 
  select(src_subject_id, eventname, sex, smri_vol_scs_intracranialv)

  #rename ICV variable so naming consistent with imaging prep script
  ICV <- ICV %>% 
    rename(ICV = smri_vol_scs_intracranialv)
...
covs_all <- covs_all %>% 
  left_join(y=dti, by= c("src_subject_id","eventname","sex")) #merge with dti motion
...
covs_all <- covs_all %>% 
  left_join(y=ICV, by= c("src_subject_id","eventname","sex")) #merge with ICV
...
#Age --- looks good, NAs = 2
summary(covs_y0$age_years) 
