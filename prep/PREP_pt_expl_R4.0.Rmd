...
```{r, pt gonadal average}

#males 
pt_m_lm <- lm(gonad_M ~age_years, na.action=na.exclude, data = M_pds_timing) # create linear model, and specify na.exclude so that df size stays the same.
summ(pt_m_lm,
     robust = TRUE, #robust standard error = HC3 as per sandwich package
     scale = TRUE,  #scaled beta used
     confint = TRUE, digits = 3) #get confidence intervals. 95% CI by default 
...
M_pds_timing$gonad_pt_m <- resid(pt_m_lm)
summ_g_m <- summary(M_pds_timing$gonad_pt_m)
summ_g_m
...
#### Female pubertal timing linear model ####
#model summary
pt_f_lm <- lm(gonad_F ~age_years, na.action=na.exclude, data = F_pds_timing) # create linear model, and specify na.exclude so that df size stays the same.
summ(pt_f_lm,
     robust = TRUE, #robust standard error = HC3 as per sandwich package
     scale = TRUE,  #scaled beta used
     confint = TRUE, digits = 3) #get confidence intervals. 95% CI by default 
...
F_pds_timing$gonad_pt_f <- resid(pt_f_lm)
summ_g_f <- summary(F_pds_timing$gonad_pt_f)
summ_g_f 
