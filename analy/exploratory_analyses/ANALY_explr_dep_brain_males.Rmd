...
ABCD Exploratory Analysis Script - Depression ~ Brain Structure -> Males
...
##STEP 2: Model set up We are the reg_phewasStyle_GLMM.R function to run the mixed effects models. Note that we will use an average measure across the left and right hemispheres for bilateral brain regions. 
...
ls.dep.cate=unique(target.model$factor)
...
ls.model.region.covWholeB$covs[!grepl('FA\\.|MD\\.',ls.model.region.covWholeB$factor)]=
      paste0(ls.model.region.covWholeB$covs[!grepl('FA\\.|MD\\.' ,ls.model.region.covWholeB$factor)],'+ WBV')
...
#Global (bulk) models
result.males.dep_brain.bulk=reg_phewasStyle(ls.model.bulk,dat_short=targetdata,correctByFactor = T)

#Regional (Individual) models
result.males.dep_brain.region=reg_phewasStyle(ls.model.region,dat_short=targetdata, correctByFactor = T)

#covary for WBV in cortical and subcort models
result.males.dep_brain.region.covWholeB =reg_phewasStyle(ls.model.region.covWholeB,dat_short=targetdata, correctByFactor = T)


#add TRUE column if p.corrected < 0.06 which means that all values of ≤0.05

#global results - none significant
result.males.dep_brain.bulk$Sig <- as.numeric((as.character(result.males.dep_brain.bulk[,"p.corrected"]))) < 0.06

#regional results
result.males.dep_brain.region$Sig <- as.numeric((as.character(result.males.dep_brain.region[,"p.corrected"]))) < 0.06

#cov whole brain results 
result.males.dep_brain.region.covWholeB$Sig <- as.numeric((as.character(result.males.dep_brain.region.covWholeB[,"p.corrected"]))) < 0.06


# #Save as R.Data file
save(result.males.dep_brain.bulk,result.males.dep_brain.region,file='/Volumes/GenScotDepression/users/niamh/puberty_ABCD/ABCD_puberty_depression/results/exploratory_analysis/males_dep_brain_explr.RData')

#Save results as .csv files

#bulk results
write.csv(result.males.dep_brain.bulk, file = '/Volumes/GenScotDepression/users/niamh/puberty_ABCD/ABCD_puberty_depression/results/exploratory_analysis/sig.males.dep.brain.bulk.csv')

#regional results
write.csv(result.males.dep_brain.region, file = '/Volumes/GenScotDepression/users/niamh/puberty_ABCD/ABCD_puberty_depression/results/exploratory_analysis/sig.males.dep.brain.regional.csv')

#cov for WBV
write.csv(result.males.dep_brain.region.covWholeB, file = '/Volumes/GenScotDepression/users/niamh/puberty_ABCD/ABCD_puberty_depression/results/exploratory_analysis/sig.males.dep.brain.regional.covWholeB.csv')
