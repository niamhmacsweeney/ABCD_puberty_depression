...
ls.factor=colnames(targetdata)[grep('^pt_f',colnames(targetdata))]
```
...
#Global (bulk) models
result.females.pt_brain.bulk=reg_phewasStyle(ls.model.bulk,dat_short=targetdata, correctByFactor = T)

#Regional (Individual) models
result.females.pt_brain.region=reg_phewasStyle(ls.model.region,dat_short=targetdata, correctByFactor = T)

#covary for WBV
result.females.pt_brain.region.covWholeB=reg_phewasStyle(ls.model.region.covWholeB,dat_short=targetdata, correctByFactor = T)


#add TRUE column if p.corrected < 0.002 which means that all values of ≤0.001

#global results - 
result.females.pt_brain.bulk$Sig <- as.numeric((as.character(result.females.pt_brain.bulk[,"p.corrected"]))) < 0.002


#regional results
result.females.pt_brain.region$Sig <- as.numeric((as.character(result.females.pt_brain.region[,"p.corrected"]))) < 0.002

#Save as R.Data file
save(result.females.pt_brain.bulk,result.females.pt_brain.region, result.females.pt_brain.region.covWholeB, file='/Volumes/GenScotDepression/users/niamh/puberty_ABCD/ABCD_puberty_depression/results/exploratory_analysis/females_pt_brain_explr.RData')

#Save results as .csv files

#global results
write.csv(result.females.pt_brain.bulk, file = '/Volumes/GenScotDepression/users/niamh/puberty_ABCD/ABCD_puberty_depression/results/exploratory_analysis/sig.females.pt.brain.bulk.csv')

#regional results 
write.csv(result.females.pt_brain.region, file = '/Volumes/GenScotDepression/users/niamh/puberty_ABCD/ABCD_puberty_depression/results/exploratory_analysis/sig.females.pt.brain.regional.csv')

#regional results + WBV
write.csv(result.females.pt_brain.region.covWholeB, file = '/Volumes/GenScotDepression/users/niamh/puberty_ABCD/ABCD_puberty_depression/results/exploratory_analysis/sig.females.pt.brain.regional.covWholeB.csv')
...
CCfitBase <- lmer(Vsc.aa ~ pt_f + age_y3 + race_y0
                            + (1|scnr_id) + (1|fam_id),
