...
#####STEP 2: Generate male sample only 

```{r, generate males only df}

targetdata <- df

#check males and females breakdown
targetdata %>% 
  dplyr::count(sex)
...
#add TRUE column if nominal p value < 0.0006 for pilot ROIs, which means that all values of ≤0.0005... will be included due to the large number of decimal places. 

#global results
result.males.pt_brain.global$Sig <- as.numeric((as.character(result.males.pt_brain.global[,"p.value"]))) < 0.0002

#regional results
result.males.pt_brain.region.covWholeB$Sig <- as.numeric((as.character(result.males.pt_brain.region.covWholeB[,"p.value"]))) < 0.0002

#Save results as .csv files

#global results
write.csv(result.males.pt_brain.global, file = '../results/revised_pilot/tables/supplementary_data/pilot_males.pt_brain.bulk.csv')

#regional results 
write.csv(result.males.pt_brain.region.covWholeB, file = '../results/revised_pilot/tables/supplementary_data/pilot_males_pt_brain.regional.csv')

```

STEP 6: Identify ROIs for confirmatory analysis. Threshold: Nominal p-value ≤ 0.0002
