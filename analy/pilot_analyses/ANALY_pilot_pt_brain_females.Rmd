...
The purpose of this script is to run the pilot analysis (Pubertal timing - brain structure) for the Puberty RR. For this analysis, we will be using the baseline ABCD data N=9,339, males= 4802, females = 4537, to generate ROIs for the puberty - brain structure - depression mediation analysis that constitutes H2 and H3 in our main analyses. 
...
#####STEP 2: Generate female only sample for pilot analysis. 
```{r, generate females only df}
...
#add TRUE column if nominal p value < 0.0002 for pilot ROIs, which means that all values of ≤0.0001... will be included due to the large number of decimal places. 

#global results
result.females.pt_brain.global$Sig <- as.numeric((as.character(result.females.pt_brain.global[,"p.value"]))) < 0.0002

#regional results
result.females.pt_brain.region.covWholeB$Sig <- as.numeric((as.character(result.females.pt_brain.region.covWholeB[,"p.value"]))) < 0.0002

#Save results as .csv files

#global results
write.csv(result.females.pt_brain.global, file = '../results/revised_pilot/tables/supplementary_data/pilot_females.pt_brain.global.csv')

#regional results 
write.csv(result.females.pt_brain.region.covWholeB, file = '../results/revised_pilot/tables/supplementary_data/pilot_females.pt_brain.regional.csv')


```

###STEP 6: Identify ROIs for confirmatory analysis. Threshold: Nominal p-value \< 0.0001

Note: The threshold is indicated as <0.0002 to account for the large number of decimal places given for p-values.
```{r, make tables}


ROI_pt_brain_females_global_results <- result.females.pt_brain.global %>%
  filter(p.value <0.0002)
