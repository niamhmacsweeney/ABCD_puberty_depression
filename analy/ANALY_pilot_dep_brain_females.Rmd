...
As this sample includes related individuals, we will need to account for the nested structure of the data in our models. To do this we will nest family within scanner(detailed further below). The pilot analysis will use a basic model, outlined below.
...
#####STEP 2: Generate subsample (10% of unrelated sample) for pilot analysis. N=1188 Get females only
...
#get mean age and sd for pilot sample. Mean age = 9.92, SD = 0.62
...
Covariates: Fixed = age, ethnicity; Random: ID and family ID nested within scanner ID
...
Note: for the lme models (bilaterlal brain regions only), the data needs to be in long-format.  Hemisphere is controlled for as a fixed, within-participant effect.
...
source('../functions/reg_phewasStyle_NestedRandom.R')
