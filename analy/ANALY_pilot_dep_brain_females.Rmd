...
#Trouble shooting: remove 20 variables causing multicollinearity issues: superior longitudinal fasiciculus, superior corticostriate, and keep the temporal and parietal SLF, and fronto parietal SCS segementations of these more broadly defined tracts:
"rh.dtifa_slf"  "lh.dtifa_slf"  "lh.dtifa_tslf" "rh.dtifa_pslf" "rh.dtifa_scs"  "lh.dtifa_scs"  "rh.dtifa_pscs" "lh.dtifa_pscs" "rh.dtimd_slf" "lh.dtimd_slf"  "rh.dtimd_tslf" "lh.dtimd_tslf" "rh.dtimd_pslf" "lh.dtimd_pslf" "rh.dtimd_scs"  "lh.dtimd_scs"  "rh.dtimd_fscs" "lh.dtimd_fscs" "rh.dtimd_pscs" "lh.dtimd_pscs" - MODEL STILL GIVING SINGULARITY ISSUES.
...
ls.dep.fs.long=colnames(dat_long)[grep('sa\\.|thk\\.|vol\\.|sulc\\.|\\.ASEG\\.|avg\\.dtifa\\_|avg\\.dtimd\\_|\\_allfibers|\\_cc|\\_fmaj|\\_fmin',colnames(dat_long))] #select vars of interest
...
ls.dep.all=ls.dep.all[c(grep('sa\\.|thk\\.|vol\\.|sulc\\.',ls.dep.all),
                        ...
                        grep('avg\\.dtifa\\_',ls.dep.all),
                        grep('avg\\.dtimd\\_',ls.dep.all),
                        grep('\\_allfibers',ls.dep.all),
                        grep('\\_cc',ls.dep.all),
                        grep('\\_fmaj',ls.dep.all),
                        grep('\\_fmin',ls.dep.all))]
ls.dep.all=ls.dep.all[!duplicated(ls.dep.all)]

#ls.dep.fs.long=colnames(dat_long)[-grep('\\_slf|\\_tslf|\\_pslf|\\_scs|\\_pscs|\\_fscs|\\_fxcut|\\_fx|\\_sifc|\\_ifsfc|\\_cst',colnames(dat_long))] #remove multicolin. vars
...
ls.models$covs[!grepl('bl\\.|dtifa\\_|dtimd\\_',ls.models$dependent)]=paste0(ls.models$covs[!grepl('bl\\.|dtifa\\_|dtimd\\_',ls.models$dependent)],'+hemi')
...
#ls.models$covs[!grepl('bl\\.|avg\\.dtifa\\_|avg\\.dtimd\\_',ls.models$dependent)]=paste0(ls.models$covs[!grepl('bl\\.|avg\\.dtifa\\_|avg\\.dtimd\\_',ls.models$dependent)],'+hemi')

#add dti mean motion (fd) as cov for dti models
# ls.models$covs[grepl('avg\\.dtifa\\_|avg\\.dtimd\\_',ls.models$dependent)]=paste0(ls.models$covs[grepl('avg\\.dtifa\\_|avg\\.dtimd\\_',ls.models$dependent)],'+dmri_dti_meanmotion')
