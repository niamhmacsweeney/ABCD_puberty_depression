...
sd(df$age_years, na.rm=T) #SD = 0.617
...
We need to change dataframe from "df" to "targetdata" for function setup. 
We need to change "src_subject_id" to "f.eid" so that function will run.

```{r, colnames}

targetdata <- df #change object name
...
Define functions
Note: this Phewas function includes confidence intervals as an inference criteria output. This will appear as "run model" in the Functions section.
...
#Save as R.Data file
save(result.females.PT_brain.bulk,result.females.PT_brain.region.covWholeB,file='../results/PT_females_brain_pilot_analysis_withCI.RData')
...
#global results
write.csv(result.females.PT_brain.bulk, file = "../results/tables/pilot_analy_females.PT_brain.bulk.csv")
