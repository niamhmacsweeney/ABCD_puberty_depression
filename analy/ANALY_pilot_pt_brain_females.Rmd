---
title: "ANALY_pilot_pt_brain_females"
...
\#ABCD Pilot Analysis Script - Pubertal timing \~ Brain Structure -\> Females
...
\#\#\#\#Independent Variable Pubertal timing: pds_timing_INT_cleaned_R3.0.rds + hormones_cleaned_R3.0.rds
...
#### Workflow 1. Setup: Load data, merge to create master dataframe, keep unrelated participants only 2. Pilot prep: Get subsample of 10% of unrelated sample (N= 9,775). Split into males and females. 3. Set up lme function using Shen's template script
...
df <- df %>% 
  dplyr::select(-age_years.x, -eventname.x, -eventname.y, -sex.x, -sex.y)

#rename year variable
df <- df  %>% 
  dplyr::rename(age_years = age_years.y)
...
Reduce to unrelated sample only Before N=11766, After N=9,775
...
nrow(unrel_df) #check that N= 9775
...
Get sample demographics: MAge = 9.90, SD= 0.62
...
#also want SD. Note: need na.rm=T otherwise returns NA. 
sd(unrel_df$age_years, na.rm=T) #SD = 0.617

```

#### STEP 2: Generate subsample (10% of unrelated sample) for pilot analysis. N=978 Get females only
...
#Get female sample only; F: N= 456
unrel_df <- unrel_df %>% 
  filter(sex == "F")

```

#### STEP 3: Model set up We are using X.Shen's reg_phewasStyle_withCI.R function to run the linear models.

We need to change dataframe from "df" to "targetdata" for function setup. We need to change "src_subject_id" to "f.eid" so that function will run.

```{r, colnames}

targetdata <- unrel_df #change object name
...
Define functions Note: this Phewas function includes confidence intervals as an inference criteria output. This will appear as "run model" in the Functions section.

```{r, Phewas function, with CI}
...
ls.factor=colnames(targetdata)[grep('dhea_timing|test_timing|estr_timing|pt_f_INT',colnames(targetdata))]
...
#Global (bulk) models
result.females.PT_brain.bulk=reg_phewasStyle(ls.model.bulk,dat_short=targetdata,dat_long=dat_long,correctByFactor = T)

#Regional (Individual) models
result.females.PT_brain.region.covWholeB=reg_phewasStyle(ls.model.region.covWholeB,dat_short=targetdata,dat_long=dat_long,correctByFactor = T)

#Save as R.Data file
save(result.females.PT_brain.bulk,result.females.PT_brain.region.covWholeB,file='../results/PT_females_brain_pilot_analysis.RData')

#add TRUE column if nominal p value < 0.005 for pilot ROIs

#global results
result.females.PT_brain.bulk$Sig <- as.numeric((as.character(result.females.PT_brain.bulk[,"p.value"]))) < 0.005

#regional results
result.females.PT_brain.region.covWholeB$Sig <- as.numeric((as.character(result.females.PT_brain.region.covWholeB[,"p.value"]))) < 0.005

#Save results as .csv files

#global results
write.csv(result.females.PT_brain.bulk, file = "../results/tables/pilot_analy_females_PT_brain_global.csv")

#regional results 
write.csv(result.females.PT_brain.region.covWholeB, file = "../results/tables/pilot_analy_females.PT_brain.regional.csv")

```

#### STEP 6: Identify ROIs for confirmatory analysis. Threshold: Nominal p-value =\< 0.005

Only regional measures met criteria to be classified as an ROI.

```{r, make tables}

ROI_PT_brain_females_regional_results <- result.females.PT_brain.region.covWholeB   %>% 
  filter(p.value <0.005)

# #want to get unique brain measures across PT measures
# tmp = ROI_PT_brain_females_regional_results
# 
# #create new column with brain measure only (i.e, not metric or FS name)
#   tmp$brainstructure = sub(".*RC\\.|.*EG\\.|.*fa_|.*md_", "", tmp$dependent) 
# 
#   tmp %>% 
#   count(brainstructure) #count number of brain measures (ROIs) total
# unique(tmp$brainstructure) #get unique brain measures 
 

#Save as R.Data file
save(ROI_PT_brain_females_regional_results,file='../results/ROIs_PT_females_brain_pilot_analysis_withCI.RData')

#save as .csv
write.csv(ROI_PT_brain_females_regional_results, file = "../results/tables/ROIs_PT_brain_females.csv")

# ROI_females_regional %>% 
