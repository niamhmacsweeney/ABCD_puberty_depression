run_model <- function(ls.mod,mod.dat_short,mod.dat_long){
   ...
   if (mod.type=='lme'){
      ...
      fit=lme(as.formula(as.character(mod)),data=mod.dat_long,
              na.action=na.exclude,random=~1|f.eid |rel_family_id/scanner_id,control=lmeControl(opt = "optim"))
      ...
      if (length(table(dep.dat))==2){
         ...
         fit=glmer(as.formula(as.character(mod) + rel_family_id | scanner_id),data=mod.dat_short,na.action=na.exclude,family = 'poisson')
      }else{
         mod=paste0('scale(',mod.dep,')~',mod.covs,'+scale(',mod.factor,')')
         fit=glmer(as.formula(as.character(mod) + rel_family_id | scanner_id),data=mod.dat_short,na.action=na.exclude)
      }            
