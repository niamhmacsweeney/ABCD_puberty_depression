...
run_model <- function(ls.mod,mod.dat_short,mod.dat_long){
   ...
   if (mod.type=='bilateral'){
      ...
      mod=paste0(mod.dep,'~',mod.covs,'+(1|scanner_id) + (1|rel_family_id:f.eid)+',mod.factor)
      ...
      if (length(table(dep.dat))==2){
         mod=paste0(mod.dep,'~',mod.covs,'+(1|scanner_id) + (1|rel_family_id:f.eid)+scale(',mod.factor,')')
         ...
         mod=paste0('scale(',mod.dep,')~',mod.covs,'+(1|scanner_id) + (1|rel_family_id:f.eid)+scale(',mod.factor,')')
